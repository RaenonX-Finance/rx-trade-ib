...
class IBapiBot(EWrapper, EClient):
    ...
    def trade_on_px_data(
        ...
        """
        ``duration`` can be set up to 1 week, using S, D, or W as unit. Default to S if not specified.
        Example values: 1 S (1 sec), 5 D (5 days), 3600 S (1 hour), 600 (10 mins).

        ``bar_size`` has the following known valid values:
        ...
        - 1 day
        """
        # Do not register the event handler until the position data is ready
        self.refresh_positions()
        while self._position_data is None:
            time.sleep(0.1)

        def decorator(on_px_data_updated: OnPxDataUpdated) -> None:
            self.request_px_data(
                contract=contract,
                duration=duration,
                bar_size=bar_size,
