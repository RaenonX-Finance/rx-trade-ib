...
def _enter(
    ...
    if has_continuous_2_extrema(px_data, PxDataCol.LOCAL_MIN, PxDataCol.LOCAL_MAX, limit_count=5):
        ...
        print(f"ET - Double top LONG @ {desired_px}")
        account.long(contract, quantity, desired_px)
    elif (
            candle_prev[PxDataCol.CLOSE] < candle_prev_2[PxDataCol.OPEN]
            and account.short(contract, quantity, desired_px)
    ):
        print(f"ET - K short @ {desired_px}")
    elif (
            candle_prev[PxDataCol.CLOSE] > candle_prev_2[PxDataCol.OPEN]
            and account.long(contract, quantity, desired_px)
    ):
        print(f"ET - K long @ {desired_px}")
